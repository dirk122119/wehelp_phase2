...
def registerUserAPI():
	...
	if not(name) or not(email) or not(password):
		response = make_response(jsonify({"error":True,"message":"三者不能空白"}),400,{'content-type':'application/json','Access-Control-Allow-Origin':"*","Access-Control-Allow-Methods":"*"})
		return response
	...
	if(result!=None):
		response = make_response(jsonify({"error":True,"message":"重複的email"}),400,{'content-type':'application/json','Access-Control-Allow-Origin':"*","Access-Control-Allow-Methods":"*"})
		cursor.close()
		connect_objt.close()
		return response
	else:
		...
		cursor.close()
		connect_objt.close()
		response = make_response(jsonify({"ok":True}),200,{'content-type':'application/json','Access-Control-Allow-Origin':"*","Access-Control-Allow-Methods":"*"})
		return response
