class bookingPage extends HTMLElement {
  ...
  creditCardInfoFunction() {
    ...
    this.cardNumber.className = "creditCard";
    this.cardNumberLabel = document.createElement("label");
    this.cardNumberLabel.className = "label";
    this.cardNumberLabel.for = "cardNumberInput";
    this.cardNumberLabel.textContent = "卡片號碼：";
    this.cardNumberInput = document.createElement("input");
    this.cardNumberInput.id = "cardNumberInput";
    this.cardNumberInput.className = "input";
    this.cardNumberInput.placeholder = "**** **** **** ****";
    this.cardNumberInput.maxLength = "19";
    this.cardNumberInput.addEventListener("keypress", (e) => {
      if (e.keyCode < 47 || e.keyCode > 57) {
        alert("只能填數字");
        e.preventDefault();
      }

      let len = this.cardNumberInput.value.length + 1;
      if (len === 5 || len === 10 || len === 15) {
        this.cardNumberInput.value += " ";
      }
    });
    this.cardNumber.appendChild(this.cardNumberLabel);
    this.cardNumber.appendChild(this.cardNumberInput);

    this.expNumber = document.createElement("div");
    this.expNumber.className = "creditCard";
    this.expNumberLabel = document.createElement("label");
    this.expNumberLabel.className = "label";
    this.expNumberLabel.for = "expNumberInput";
    this.expNumberLabel.textContent = "過期時間：";
    this.expNumberInput = document.createElement("input");
    this.expNumberInput.id = "expNumberInput";
    this.expNumberInput.className = "input";
    this.expNumberInput.placeholder = "MM/YY";
    this.expNumberInput.maxLength = "5";
    this.expNumberInput.addEventListener("keypress", (e) => {
      if (e.keyCode < 47 || e.keyCode > 57) {
        alert("只能填數字");
        e.preventDefault();
      }
      let len = this.expNumberInput.value.length + 1;
      if (len !== 3) {
        if (e.keyCode == 47) {
          e.preventDefault();
        }
      ...
      if (len === 3) {
        if (e.keyCode !== 47) {
          this.expNumberInput.value += "/";
        }
      }
    });

    this.expNumber.appendChild(this.expNumberLabel);
    this.expNumber.appendChild(this.expNumberInput);

    this.cvvNumber = document.createElement("div");
    this.cvvNumber.className = "creditCard";
    this.cvvNumberLabel = document.createElement("label");
    this.cvvNumberLabel.className = "label";
    this.cvvNumberLabel.for = "cvvNumberInput";
    this.cvvNumberLabel.textContent = "驗證密碼：";
    this.cvvNumberInput = document.createElement("input");
    this.cvvNumberInput.id = "cvvNumberInput";
    this.cvvNumberInput.className = "input";
    this.cvvNumberInput.placeholder = "CVV";
    this.cvvNumberInput.maxLength = "3";
    this.cvvNumberInput.addEventListener("keypress", (e) => {
      if (e.keyCode < 47 || e.keyCode > 57) {
        alert("只能填數字");
        e.preventDefault();
      }
    });
    this.cvvNumber.appendChild(this.cvvNumberLabel);
    this.cvvNumber.appendChild(this.cvvNumberInput);
